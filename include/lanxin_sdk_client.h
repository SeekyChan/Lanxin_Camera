...
struct SdkStats {
    ...
    SdkStats() : image_count(0), detection_count(0), null_detection_count(0), decode_failure_count(0)
    {
    }
};
...
class LanxinSdkClient
{
   ...
   private:
    // SDK 回调由厂商内部线程触发，只能通过这个上下文访问少量线程安全状态。
    struct CallbackContext {
        ...
        // 每次 start/stop 都递增，用于丢弃重启边界上的旧回调结果。
    ...
    void StopLocked();
    bool CheckSdkCall(VmrQrCameraExc code, const std::string& action, std::string* error) const;

    // 只保护 SDK handle 生命周期；SDK 回调不拿这个锁，避免回调线程被生命周期操作阻塞。
