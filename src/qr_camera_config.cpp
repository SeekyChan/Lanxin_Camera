...
template <typename T>
bool CheckRange(const std::string& name, const T& value, const T& min_value,
                const T& max_value, std::string* error) {
  if (value >= min_value && value <= max_value) {
    return true;
  }

  std::ostringstream oss;
  oss << "param '" << name << "' out of range: expected [" << min_value << ", "
      << max_value << "], got " << value;
  if (error) {
    *error = oss.str();
  }
...
bool CheckEncodeType(const std::string& name, int value, std::string* error) {
  ...
  std::ostringstream oss;
  oss << "param '" << name << "' only supports 1(Lanxin) or 5(Pepperl+Fuchs), got "
      << value;
  if (error) {
    *error = oss.str();
  }
...
bool ValidateOneCameraParams(const common_msgs::config_lx_camera& params,
                             const std::string& prefix, bool up,
                             std::string* error) {
  const int exposure =
      up ? params.up_qr_camera_param_exposure : params.down_qr_camera_param_exposure;
  const int gain = up ? params.up_qr_camera_param_gain : params.down_qr_camera_param_gain;
  const int led = up ? params.up_qr_camera_param_led_brightness
  ...
  const double qr_size =
      up ? params.up_qr_code_param_qr_size : params.down_qr_code_param_qr_size;
  const double qr_interval = up ? params.up_qr_code_param_qr_interval
                                : params.down_qr_code_param_qr_interval;
  const double radius_big =
      up ? params.up_qr_code_param_radius_big : params.down_qr_code_param_radius_big;
  const double radius_small = up ? params.up_qr_code_param_radius_small
                                 : params.down_qr_code_param_radius_small;
  const double circle_dis =
      up ? params.up_qr_code_param_circle_dis : params.down_qr_code_param_circle_dis;
  const double point_height = up ? params.up_qr_code_param_point_height
                                 : params.down_qr_code_param_point_height;
  const double big_circle_width = up ? params.up_qr_code_param_big_circle_width
                                     : params.down_qr_code_param_big_circle_width;
  const int qr_need =
      up ? params.up_qr_code_param_qr_need : params.down_qr_code_param_qr_need;
  const int dm_symbol =
      up ? params.up_qr_code_param_dm_symbol : params.down_qr_code_param_dm_symbol;
  const double decode_timeout = up ? params.up_qr_code_param_decode_timeout
                                   : params.down_qr_code_param_decode_timeout;
  const int encode_type =
      up ? params.up_qr_code_param_encode_type : params.down_qr_code_param_encode_type;
  ...
  return CheckRange(prefix + "_qr_camera_param_exposure", exposure, 1, 2000, error) &&
         ...
         CheckRange(prefix + "_qr_code_param_qr_interval", qr_interval, 0.0, 0.1,
                    error) &&
         CheckRange(prefix + "_qr_code_param_radius_big", radius_big, 0.0, 0.2,
                    error) &&
         CheckRange(prefix + "_qr_code_param_radius_small", radius_small, 0.0, 0.2,
                    error) &&
         CheckRange(prefix + "_qr_code_param_circle_dis", circle_dis, 0.0, 0.2,
                    error) &&
         CheckRange(prefix + "_qr_code_param_point_height", point_height, 0.001, 0.2,
                    error) &&
         CheckRange(prefix + "_qr_code_param_big_circle_width", big_circle_width, 0.0,
                    0.2, error) &&
         ...
         CheckRange(prefix + "_qr_code_param_decode_timeout", decode_timeout, 1.0,
                    200.0, error) &&
...
void WriteYamlValue(std::ostream& out, const std::string& name,
...
void ReadLxCameraParams(ros::NodeHandle& nh, common_msgs::config_lx_camera* params) {
  nh.param("down_qr_camera_ip", params->down_qr_camera_ip,
           params->down_qr_camera_ip);
  nh.param("down_qr_camera_param_exposure", params->down_qr_camera_param_exposure,
           params->down_qr_camera_param_exposure);
  nh.param("down_qr_camera_param_gain", params->down_qr_camera_param_gain,
           params->down_qr_camera_param_gain);
  nh.param("down_qr_camera_param_led_brightness",
           params->down_qr_camera_param_led_brightness,
           params->down_qr_camera_param_led_brightness);

  nh.param("up_qr_camera_ip", params->up_qr_camera_ip, params->up_qr_camera_ip);
  nh.param("up_qr_camera_param_exposure", params->up_qr_camera_param_exposure,
           params->up_qr_camera_param_exposure);
  nh.param("up_qr_camera_param_gain", params->up_qr_camera_param_gain,
           params->up_qr_camera_param_gain);
  nh.param("up_qr_camera_param_led_brightness",
           params->up_qr_camera_param_led_brightness,
           params->up_qr_camera_param_led_brightness);

  nh.param("down_qr_code_param_rows", params->down_qr_code_param_rows,
           params->down_qr_code_param_rows);
  nh.param("down_qr_code_param_cols", params->down_qr_code_param_cols,
           params->down_qr_code_param_cols);
  nh.param("down_qr_code_param_qr_size", params->down_qr_code_param_qr_size,
           params->down_qr_code_param_qr_size);
  ...
  nh.param("down_qr_code_param_point_height", params->down_qr_code_param_point_height,
           params->down_qr_code_param_point_height);
  nh.param("down_qr_code_param_big_circle_width",
           params->down_qr_code_param_big_circle_width,
           params->down_qr_code_param_big_circle_width);
  nh.param("down_qr_code_param_qr_need", params->down_qr_code_param_qr_need,
           params->down_qr_code_param_qr_need);
  nh.param("down_qr_code_param_dm_symbol", params->down_qr_code_param_dm_symbol,
           params->down_qr_code_param_dm_symbol);
  nh.param("down_qr_code_param_decode_timeout",
           params->down_qr_code_param_decode_timeout,
           params->down_qr_code_param_decode_timeout);
  nh.param("down_qr_code_param_big_circle_flag",
           params->down_qr_code_param_big_circle_flag,
           params->down_qr_code_param_big_circle_flag);
  nh.param("down_qr_code_param_small_circle_flag",
  ...
  nh.param("up_qr_code_param_rows", params->up_qr_code_param_rows,
           params->up_qr_code_param_rows);
  nh.param("up_qr_code_param_cols", params->up_qr_code_param_cols,
           params->up_qr_code_param_cols);
  nh.param("up_qr_code_param_qr_size", params->up_qr_code_param_qr_size,
           params->up_qr_code_param_qr_size);
  nh.param("up_qr_code_param_qr_interval", params->up_qr_code_param_qr_interval,
           params->up_qr_code_param_qr_interval);
  nh.param("up_qr_code_param_radius_big", params->up_qr_code_param_radius_big,
           params->up_qr_code_param_radius_big);
  nh.param("up_qr_code_param_radius_small", params->up_qr_code_param_radius_small,
           params->up_qr_code_param_radius_small);
  nh.param("up_qr_code_param_circle_dis", params->up_qr_code_param_circle_dis,
           params->up_qr_code_param_circle_dis);
  nh.param("up_qr_code_param_point_height", params->up_qr_code_param_point_height,
           params->up_qr_code_param_point_height);
  nh.param("up_qr_code_param_big_circle_width",
           params->up_qr_code_param_big_circle_width,
           params->up_qr_code_param_big_circle_width);
  nh.param("up_qr_code_param_qr_need", params->up_qr_code_param_qr_need,
           params->up_qr_code_param_qr_need);
  nh.param("up_qr_code_param_dm_symbol", params->up_qr_code_param_dm_symbol,
           params->up_qr_code_param_dm_symbol);
  ...
  nh.param("up_qr_code_param_small_circle_flag",
  ...
  nh.param("up_qr_code_param_save_video", params->up_qr_code_param_save_video,
           params->up_qr_code_param_save_video);
}
...
void ReadRuntimeOptions(ros::NodeHandle& nh, RuntimeOptions* options) {
  ...
  nh.param("placeholder_camera_id", options->placeholder_camera_id,
           options->placeholder_camera_id);
  nh.param("publish_placeholder_up", options->publish_placeholder_up,
           options->publish_placeholder_up);
  nh.param("publish_rate_hz", options->publish_rate_hz, options->publish_rate_hz);
  nh.param("stale_timeout_sec", options->stale_timeout_sec,
           options->stale_timeout_sec);
  nh.param("auto_reconnect", options->auto_reconnect, options->auto_reconnect);
  nh.param("reconnect_interval_sec", options->reconnect_interval_sec,
           options->reconnect_interval_sec);
  nh.param("update_service_name", options->update_service_name,
           options->update_service_name);
}
...
RuntimeOptions DefaultRuntimeOptions() {
  ...
  options.qr_type = 2;
  options.camera_id = 1;
  options.placeholder_camera_id = 2;
  options.publish_placeholder_up = true;
  options.publish_rate_hz = 100.0;
  options.stale_timeout_sec = 0.06;
  options.auto_reconnect = true;
  options.reconnect_interval_sec = 3.0;
  options.update_service_name = "update_qr_code_identify_lanxin_params";
  return options;
}

bool ValidateLxCameraParams(const common_msgs::config_lx_camera& params,
                            std::string* error) {
  return ValidateOneCameraParams(params, "down", false, error) &&
         ValidateOneCameraParams(params, "up", true, error);
}

bool ReadCameraConfig(ros::NodeHandle& private_nh, QrCameraConfig* config,
                      std::string* error) {
  if (!config) {
    if (error) {
  ...
  if (!CheckRange("publish_rate_hz", config->runtime.publish_rate_hz, 1.0, 500.0,
                  error) ||
      !CheckRange("stale_timeout_sec", config->runtime.stale_timeout_sec, 0.001,
                  10.0, error) ||
      !CheckRange("reconnect_interval_sec", config->runtime.reconnect_interval_sec,
                  0.1, 60.0, error) ||
      !CheckRange("camera_id", config->runtime.camera_id, 1, 127, error) ||
      !CheckRange("placeholder_camera_id", config->runtime.placeholder_camera_id, 1,
                  127, error) ||
...
bool WriteLxCameraParamsToServer(ros::NodeHandle& private_nh,
  ...
  SetParam(private_nh, "down_qr_code_param_qr_interval",
           params.down_qr_code_param_qr_interval);
  SetParam(private_nh, "down_qr_code_param_radius_big",
           params.down_qr_code_param_radius_big);
  SetParam(private_nh, "down_qr_code_param_radius_small",
           params.down_qr_code_param_radius_small);
  SetParam(private_nh, "down_qr_code_param_circle_dis",
           params.down_qr_code_param_circle_dis);
  SetParam(private_nh, "down_qr_code_param_point_height",
           params.down_qr_code_param_point_height);
  SetParam(private_nh, "down_qr_code_param_big_circle_width",
           params.down_qr_code_param_big_circle_width);
  SetParam(private_nh, "down_qr_code_param_qr_need", params.down_qr_code_param_qr_need);
  SetParam(private_nh, "down_qr_code_param_dm_symbol",
           params.down_qr_code_param_dm_symbol);
  SetParam(private_nh, "down_qr_code_param_decode_timeout",
           params.down_qr_code_param_decode_timeout);
  SetParam(private_nh, "down_qr_code_param_big_circle_flag",
           params.down_qr_code_param_big_circle_flag);
  SetParam(private_nh, "down_qr_code_param_small_circle_flag",
           params.down_qr_code_param_small_circle_flag);
  SetParam(private_nh, "down_qr_code_param_encode_type",
           params.down_qr_code_param_encode_type);
  SetParam(private_nh, "down_qr_code_param_save_video",
           params.down_qr_code_param_save_video);
  ...
  SetParam(private_nh, "up_qr_code_param_qr_interval",
           params.up_qr_code_param_qr_interval);
  SetParam(private_nh, "up_qr_code_param_radius_big", params.up_qr_code_param_radius_big);
  SetParam(private_nh, "up_qr_code_param_radius_small",
           params.up_qr_code_param_radius_small);
  SetParam(private_nh, "up_qr_code_param_circle_dis", params.up_qr_code_param_circle_dis);
  SetParam(private_nh, "up_qr_code_param_point_height",
           params.up_qr_code_param_point_height);
  SetParam(private_nh, "up_qr_code_param_big_circle_width",
           params.up_qr_code_param_big_circle_width);
  ...
  SetParam(private_nh, "up_qr_code_param_decode_timeout",
           params.up_qr_code_param_decode_timeout);
  SetParam(private_nh, "up_qr_code_param_big_circle_flag",
           params.up_qr_code_param_big_circle_flag);
  SetParam(private_nh, "up_qr_code_param_small_circle_flag",
           params.up_qr_code_param_small_circle_flag);
  SetParam(private_nh, "up_qr_code_param_encode_type",
           params.up_qr_code_param_encode_type);
...
bool WriteLxCameraParamsToDefaultFile(
    const common_msgs::config_lx_camera& params, std::string* error) {
  if (!ValidateLxCameraParams(params, error)) {
    return false;
  }
  ...
  const std::string temp_path = path + ".tmp";
  {
    ...
    WriteYamlValue(out, "down_qr_code_param_qr_size",
                   params.down_qr_code_param_qr_size);
    WriteYamlValue(out, "down_qr_code_param_qr_interval",
                   params.down_qr_code_param_qr_interval);
    WriteYamlValue(out, "down_qr_code_param_radius_big",
                   params.down_qr_code_param_radius_big);
    WriteYamlValue(out, "down_qr_code_param_radius_small",
                   params.down_qr_code_param_radius_small);
    WriteYamlValue(out, "down_qr_code_param_circle_dis",
                   params.down_qr_code_param_circle_dis);
    WriteYamlValue(out, "down_qr_code_param_point_height",
                   params.down_qr_code_param_point_height);
    WriteYamlValue(out, "down_qr_code_param_big_circle_width",
                   params.down_qr_code_param_big_circle_width);
    WriteYamlValue(out, "down_qr_code_param_qr_need",
                   params.down_qr_code_param_qr_need);
    WriteYamlValue(out, "down_qr_code_param_dm_symbol",
                   params.down_qr_code_param_dm_symbol);
    WriteYamlValue(out, "down_qr_code_param_decode_timeout",
                   params.down_qr_code_param_decode_timeout);
    WriteYamlValue(out, "down_qr_code_param_big_circle_flag",
                   params.down_qr_code_param_big_circle_flag);
    WriteYamlValue(out, "down_qr_code_param_small_circle_flag",
                   params.down_qr_code_param_small_circle_flag);
    WriteYamlValue(out, "down_qr_code_param_encode_type",
                   params.down_qr_code_param_encode_type);
    WriteYamlValue(out, "down_qr_code_param_save_video",
                   params.down_qr_code_param_save_video);
    out << "\n";
    ...
    WriteYamlValue(out, "up_qr_code_param_qr_interval",
                   params.up_qr_code_param_qr_interval);
    WriteYamlValue(out, "up_qr_code_param_radius_big",
                   params.up_qr_code_param_radius_big);
    WriteYamlValue(out, "up_qr_code_param_radius_small",
                   params.up_qr_code_param_radius_small);
    WriteYamlValue(out, "up_qr_code_param_circle_dis",
                   params.up_qr_code_param_circle_dis);
    WriteYamlValue(out, "up_qr_code_param_point_height",
                   params.up_qr_code_param_point_height);
    WriteYamlValue(out, "up_qr_code_param_big_circle_width",
                   params.up_qr_code_param_big_circle_width);
    WriteYamlValue(out, "up_qr_code_param_qr_need", params.up_qr_code_param_qr_need);
    WriteYamlValue(out, "up_qr_code_param_dm_symbol",
                   params.up_qr_code_param_dm_symbol);
    WriteYamlValue(out, "up_qr_code_param_decode_timeout",
                   params.up_qr_code_param_decode_timeout);
    WriteYamlValue(out, "up_qr_code_param_big_circle_flag",
                   params.up_qr_code_param_big_circle_flag);
    WriteYamlValue(out, "up_qr_code_param_small_circle_flag",
                   params.up_qr_code_param_small_circle_flag);
    WriteYamlValue(out, "up_qr_code_param_encode_type",
                   params.up_qr_code_param_encode_type);
    WriteYamlValue(out, "up_qr_code_param_save_video",
                   params.up_qr_code_param_save_video);
